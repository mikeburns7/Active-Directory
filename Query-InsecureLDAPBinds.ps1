...
Write-Host "Run LDAP Reg" $x
$KeyCheck = Test-Path 'HKLM:\SYSTEM\CurrentControlSet\Services\NTDS\Diagnostics' 

	if (-not $KeyCheck){
	Write-Host -f red "$ComputerName : Registry Key Not Found. Please Check for Updates"
	 Quit
	}
	...
	If ($value -lt 2){
	 ...
	 Write-host -f red "$ComputerName : Registry Key Updated To Enable LDAP Interface Logging. Please Run Again in 24 Hours to Analayze Results"
...
#function to check if registry key exists and or the proper value for LDAP Interface Logging
	...
	If ($localmachine -ne $ComputerName)
	{
	...
	$Events = Get-WinEvent -ComputerName $ComputerName -FilterHashtable @{Logname='Directory Service';Id=2889; StartTime=(get-date).AddHours("-$Hours")} -ErrorAction Stop
		...
		if ($_.Exception -match "$ComputerName : No events were found that match the specified selection criteria") {
		$Events = $null
			 }
	    }
